...
- Invalid or unavailable input: when `review_path` is concrete and writable, write the requested artifact with `Decision: INCOMPLETE` and the missing evidence; when it is not writable, write nothing. Either way return only the reviewer's exact `# Output` envelope with `Status: INCOMPLETE`. Never probe, relocate, or write any other artifact.
