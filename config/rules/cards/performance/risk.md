...
### Bounded work
Avoid unbounded work on inputs that can grow. Add pagination, limits, early exits, batching, streaming, or explicit workload bounds when needed.
...
Bad: sort all entries when only the top `limit` are used.
Good: select the top slice, then sort only the kept slice.
