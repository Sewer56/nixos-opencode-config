...
permission:
  ...
  read:
    ...
    "*.env.example": allow
  edit:
    "*PROMPT-TICKET*.draft.review-clarity.md": allow
  grep: allow
  glob: allow
  list: allow
  todowrite: allow
...
1. Load cache
- Cache: `PROMPT-TICKET-login-bug.draft.handoff.md` → `PROMPT-TICKET-login-bug.draft.review-clarity.md`. Read the cache file if it exists. Treat missing or malformed cache as empty.
