...
Group resolved paths by parent module and language. When a module directory contains source files in multiple languages, group by (parent module, language) so each language gets its own collector. Spawn one `_audit/public-api/collector` per group in a single parallel call.
