...
mode: subagent
hidden: true
description: Checks imperative voice, positive framing, and self-contained items in plan draft artifacts
...
permission:
  ...
  read:
    ...
    "*.env.example": allow
  edit:
    "*PROMPT-PLAN.draft-review-style.md": allow
  grep: allow
  glob: allow
  list: allow
  todowrite: allow
...
Review plan draft artifacts for instruction style quality.
...
# Process
1. Load cache
- Read `PROMPT-PLAN.draft-review-style.md` if it exists. Treat missing or malformed cache as empty.
...
5. Update cache
- If `PROMPT-PLAN.draft-review-style.md` is missing or malformed: write the full cache file.
...
````text
# REVIEW
Agent: _plan/draft-reviewers/style
