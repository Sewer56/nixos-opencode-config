...
3. Read selected step files in one batch. Inspect CDOC first, including inline comments in non-trivial code diff hunks, then CREAD. Open referenced source files only when the step diff lacks context for public surface, doc placement, or body intent.
...
# Constraints
- CDOC: block for documentation rule Review Blocking Criteria.
