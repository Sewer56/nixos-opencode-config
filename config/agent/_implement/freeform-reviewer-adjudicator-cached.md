...
4. Derive sidecar state paths from `state_path`:
   - `@_implement/freeform-reviewer/freeform-reviewer-a-cached`: `<state_path without .md>.a.md`
   - `@_implement/freeform-reviewer/freeform-reviewer-b-cached`: `<state_path without .md>.b.md`
   - sidecar actions: replace `.md` with `.actions.<same nnn>.md`.
5. Run `@_implement/freeform-reviewer/freeform-reviewer-a-cached` and `@_implement/freeform-reviewer/freeform-reviewer-b-cached` independently with the same inline context and separate sidecar `cache_path`/`actions_path` values.
