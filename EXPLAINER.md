...
Integration repair has two turns. Each cohort and one-shot independently resolve an explicit positive user limit, no limit as `unlimited`, otherwise five; bounded failure reports attempted and resolved values. Each turn revalidates, re-reviews, and commits exact repair paths.
...
- Repair loops default to five turns per cohort and one-shot implementation, two at final integration and in iterate, and one CodeRabbit self-fix pass with one re-review. Cohorts and one-shot independently resolve an explicit command-user positive limit or no limit.
