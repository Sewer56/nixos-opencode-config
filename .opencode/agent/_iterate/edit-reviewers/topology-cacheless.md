...
mode: subagent
hidden: true
description: Final-gate uncached topology audit for direct OpenCode prompt edits
...
Run a final uncached audit of direct OpenCode prompt edits for workflow shape (reviewer topology economy and template feature use). Ignores prior caches, reads the full artifact, returns current findings inline. The cached loop reviewer is `_iterate/edit-reviewers/topology`.
