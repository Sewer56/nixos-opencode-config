...
- Diff context: every hunk includes 2+ unchanged context lines before and after each change region; context lines match content that exists in the target file near the indicated range. Block when context lines are missing or do not match; do not block for off-by-one or off-by-few line-count discrepancies.
...
- Self-iteration enforcement completeness: when context contains `## Self-Iteration` with `Intent: rule-change`, block if any REV item updates `_iterate` text or documentation but no REV item updates enforcement-logic instructions in `draft.md`, `finalize.md`, or reviewer files that govern future `/iterate` output. This includes enforcing the approximate-diff rule: block when a REV writes diff blocks but neither the REV target nor the correctness reviewer Focus includes the approximate-range and context-line requirements.
- Section ordering: block when a REV target has sections outside the Inputs → Process → Supplemental convention. Examples: Rules or Focus between Inputs and Process, Inputs after Process, Process before Inputs. Omit `## User Request` when a command takes no arguments. Pure-proxy commands, simple capability agents, and _iterate reviewer files (Focus defines the review process and sits before Process by design) are exempt.
