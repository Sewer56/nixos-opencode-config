...
Call `_iterate/review` for required lenses with request, contract, validation, staged paths, and output path. Pass no editor narration.

Send candidates to `_iterate/verifier`. Repair accepted `TARGET` blockers only. Contract/evidence defects are `INCOMPLETE`; never repair advisories.
